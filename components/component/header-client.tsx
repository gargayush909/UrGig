...
export default function HeaderClient({ session }: { session: Session | null }) {
  ...
  return (
    <header className='sticky top-0 bg-background/80 backdrop-blur-xl'>
        ...
        <Link className="flex items-center justify-center text-2xl font-[700]" href="/">
          <UrGigLogo></UrGigLogo>
        </Link>
            ...
            <Link className="text-sm font-medium " href="/account">
              <div className='flex'>
                  ...
                  <Avatar className='select-none'>                  
                    <AvatarImage draggable="false" src={user?.user_metadata.avatar_url} alt={user?.user_metadata.full_name} className='dark:invert'/>
                    <AvatarFallback>{
                    (user?.user_metadata.fullname === undefined) ? user?.user_metadata.full_name.toString().split(' ').map((word : string) => word[0].toUpperCase()).join('') : user?.email
                    }</AvatarFallback>
                  </Avatar>
          ...
          (
            <div className='flex gap-2'>
                <Link className="text-sm font-medium hover:underline underline-offset-4" href="/signin">
                    <Button variant={"outline"} className='hover:bg-primary px-5 bg-background/0 hover:text-background'>Sign In</Button>
                </Link>
                <Link className="text-sm font-medium hover:underline underline-offset-4" href="/signup">
                    <Button variant={"outline"} className='hover:bg-primary px-5 bg-background/0 hover:text-background'>Sign Up</Button>
                </Link>
            </div>
        )
          }
